...
class ImageEncoder(torch.nn.Module):
    ...
    def __init__(self, arch):
        ...
        if arch == 'resnet18':
            ...
            self.model = torchvision.models.convnext_small(weights=None)
            self.n_features = self.model.classifier[-1].in_features
            self.model.classifier[-1] = torch.nn.Identity()
            ...
            self.model = torchvision.models.swin_v2_s(weights=None)
            ...
            self.model = timm.create_model('convnextv2_tiny.fcmae_ft_in22k_in1k', pretrained=False)
            self.n_features = self.model.head.fc.in_features
            self.model.head.fc = torch.nn.Identity()
        elif arch == 'convnextv2_tiny.fcmae':
            self.model = timm.create_model('convnextv2_tiny.fcmae', pretrained=False)
            self.n_features = 768
        else:
            import sys
